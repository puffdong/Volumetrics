...
vec2 mouse_to_uv() {
    return vec2(1.0, -1.0) * (2.0 * u_mouse_pos.xy - u_resolution.xy) / u_resolution.y;
}
...
void main() {
    ...
    float red = texture(u_src_color, uv_to_texture((uv + offset - dispersion) * 0.75)).r;
    float green = texture(u_src_color, uv_to_texture((uv + offset - dispersion * 0.5) * 0.75)).g;
    float blue = texture(u_src_color, uv_to_texture((uv + offset + dispersion) * 0.750)).b;
