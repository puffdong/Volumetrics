...
void Glass::init(ResourceManager& resources) {
    shader = new Shader(resources.get_full_path("res://shaders/glass/glass.vs"), resources.get_full_path("res://shaders/glass/glass.fs"));
...
void Glass::enqueue(Renderer& renderer, const ButtonMap& button_map) {
    if (!_visible) return;
